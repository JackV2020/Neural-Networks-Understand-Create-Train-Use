...
// === AJAX POST: only return the result ===y
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $is_ajax) {
    ...
    if (isset($_FILES['picture_file'])) {
        ...
        if (move_uploaded_file($_FILES['picture_file']['tmp_name'], $targetfile)) {
            $venvpython3 = realpath(__DIR__) . "/my_venv/bin/python3";
            $escapedScript = escapeshellarg(__DIR__ . '/test_picture_lite_8_labels.py');
            $escaped = escapeshellarg($targetfile);
            $cmd = "$venvpython3 $escapedScript $escaped 2>&1 | grep -v INFO";
            $output = shell_exec($cmd);
            $wrapped_output = wordwrap($output, 120, "\n", true);

            // split in lines
            $lines = explode("\n", $wrapped_output);

            // Filter lines with an underscore
            ...
            echo "<h3>Results:</h3><pre>$result</pre>";
            echo "<center><img src='uploads/" . htmlspecialchars($newName) . "?" . rand(100000, 999999) . "' height='320' style='border: 1px dashed black;'><center>";
        } else {
...
<body>
<center>
    <h1>Upload an image for spiral detection</h1>
    <a href="./test.php" class="custom-file-button">Example Images</a><br><br>
    <form id="uploadForm" action="" method="post" enctype="multipart/form-data">
        <label id="imageInputLabel" for="imageInput" class="custom-file-button">
          Select or take a picture
        </label>
        <input id="imageInput" type="file" name="picture_file" accept="image/*" capture="environment" required>
        <br><br>
        <button type="submit" class="custom-file-button">Send picture</button>
    </form>
    <br>
    <img id="preview" style="max-width: 300px; max-height:300px; border: 1px dashed black;display:none" />
    <br>
</center>
    <div id="php_page"></div>
